$(document).ready(function () {
    ...
    $(".button-detail").attr(
        ...
        "/" +
            lang +
            "/user/profile/detail-kontrak/detail-transaksi?contract_number=" +
            urlParams.get("contract_number")
    );
...
function contractDetail(data) {
    ...
    $(".total-installment").text(
        "Rp. " + convertInttoCurrency(data.pembiayaan)
    );
    $(".remaining-installment").text(
        "Rp. " + convertInttoCurrency(data.sisa_angsuran)
    );
    $(".this-month-bill").text(
        "Rp. " + convertInttoCurrency(data.tagihan_bulan_ini)
    );
    $(".due-date").text(due_date);
    $(".installment-per-month").text(
        "Rp. " + convertInttoCurrency(data.angsuran_per_bulan)
    );
    $(".late-charge").text(
        "Rp. " + convertInttoCurrency(data.denda_keterlambatan)
    );
}
        ...
        success: function (dataObj) {
            if (dataObj.success === true) {
                ...
                $.each(data, function (index, value) {
                    ...
                    var due_date = dueDateRaw.toLocaleDateString(
                        lang + "-" + lang,
                        options
                    );
                    var payment_date = paymentDateRaw.toLocaleDateString(
                        lang + "-" + lang,
                        options
                    );
                    var transaction_row =
                        "<tr> <td class='installment_no'></td><td class='tanggal_jatuh_tempo'></td><td class='angsuran_per_bulan'></td><td class='angsuran_telah_dibayar'></td> <td class='tanggal_pembayaran'></td> <td class='denda_keterlambatan'></td> <td class='sisa_angsuran'></td> </tr>";

                    $("tbody").append(transaction_row);
                    $("td.installment_no").last().text(value.installment_no);
                    $("td.tanggal_jatuh_tempo").last().text(due_date);
                    $("td.angsuran_per_bulan")
                        .last()
                        .text(convertInttoCurrency(value.angsuran_per_bulan));
                    $("td.angsuran_telah_dibayar")
                        .last()
                        .text(
                            convertInttoCurrency(value.angsuran_telah_dibayar)
                        );
                    $("td.tanggal_pembayaran")
                        .last()
                        .text(dateValidChecker(payment_date));
                    $("td.denda_keterlambatan")
                        .last()
                        .text(convertInttoCurrency(value.denda_keterlambatan));
                    $("td.sisa_angsuran")
                        .last()
                        .text(convertInttoCurrency(value.sisa_angsuran));

                    total_installments += value.angsuran_per_bulan;
                    total_late_charge += value.denda_keterlambatan;
                });
                var total_transaction_row =
                    "<tr class='total'> <td></td><td>Total</td><td class='total_installment'></td><td></td><td></td><td class='total_late_charge'></td><td></td></tr>";
                $("tbody").append(total_transaction_row);

                $("td.total_installment").text(
                    convertInttoCurrency(total_installments)
                );
                $("td.total_late_charge").text(
                    convertInttoCurrency(total_late_charge)
                );
            }
        },
    });
        ...
        success: function (dataObj) {
            if (dataObj.success === true) {
                ...
                $("article.title > h3").text(
                    data.category_desc + " " + data.product_desc
                );
                $(".product").text(data.product_desc);
                $(".contract-number").text(data.contract_number);
                $(".name").text(data.nama_pemohon);
                $(".total-installment-contract").text(
                    "Rp. " + convertInttoCurrency(data.pembiayaan)
                );
                $(".due-date-contract").text(due_date.getDate());
                $(".installment-per-month-contract").text(
                    "Rp. " + convertInttoCurrency(data.angsuran_per_bulan)
                );
                $(".jangka-on-month").text(
                    data.jangka_waktu_on_month + " Bulan"
                );
                ...
                if (data.product_desc == "Sertifikat Rumah") {
                    ...
                    data.product_desc == "Alat Berat & Mesin Refinancing"
                ) {
    ...
    $.ajax({
        ...
        success: function (dataObj) {
            if (dataObj.success === true) {
                ...
                $(".building-area")
                    .text(data.luas_bangunan)
                    .append("m<sup>2</sup>");
                $(".surface-area")
                    .text(data.luas_tanah)
                    .append("m<sup>2</sup>");
