...
(function($) {
  ...
  $(document).bind("ajaxStart", function () {
                ...
                if (data.success === "1") {
                    if (credType === "motor" || credType === "mobil") {
                        if (data.data.is_branch) {
                            credits.tempat_tinggal.provinsi = htmlEntities(
                                provinsi
                            );
                            credits.tempat_tinggal.kota = htmlEntities(kota);
                            credits.tempat_tinggal.kecamatan = htmlEntities(
                                kecamatan
                            );
                            credits.tempat_tinggal.kelurahan = htmlEntities(
                                kelurahan
                            );
                            credits.tempat_tinggal.kode_pos = htmlEntities(
                                kode_pos_text
                            );
                            credits.tempat_tinggal.alamat = htmlEntities(
                                alamat
                            );
                        } else {
                            showTab2();
            ...
            success: function(result) {
                if (result.success === "1") {
                    ...
                    $("#status_sertificate").select2({
                        placeholder: $("#status_sertificate").attr(
                            "placeholder"
                        ),
                ...
                if (data.success === "1") {
                    if (credType === "motor" || credType === "mobil") {
                        if (data.data.is_pricing) {
                            credits.kendaraan.merk_kendaraan = htmlEntities(
                                merk_kendaraan
                            );
                            credits.kendaraan.merk_kendaraan_text = htmlEntities(
                                merk_kendaraan_text
                            );
                            credits.kendaraan.model_kendaraan = htmlEntities(
                                model_kendaraan
                            );
                            credits.kendaraan.model_kendaraan_text = htmlEntities(
                                model_kendaraan_text
                            );
                            credits.kendaraan.tahun_kendaraan = htmlEntities(
                                tahun_kendaraan
                            );
                            credits.kendaraan.tahun_kendaraan_text = htmlEntities(
                                tahun_kendaraan_text
                            );
    ...
    function pushDataBangunan(cb) {
        if ($("#status_sertificate").length > 0) {
            ...
            status_sertificate = $(
                "#status_sertificate option[value='" + status_sertificate + "']"
            ).text();
            provinsi_sertificate = $(
                "#provinsi_sertificate option[value='" +
                    provinsi_sertificate +
                    "']"
            ).text();
            kota_sertificate = $(
                "#kota_sertificate option[value='" + kota_sertificate + "']"
            ).text();
            kecamatan_sertificate = $(
                "#kecamatan_sertificate option[value='" +
                    kecamatan_sertificate +
                    "']"
            ).text();
            kelurahan_sertificate = $(
                "#kelurahan_sertificate option[value='" +
                    kelurahan_sertificate +
                    "']"
            ).text();
            kode_pos_sertificate = $("#kode_pos_sertificate").val();
            own_sertificate = $(
                "#own_sertificate option[value='" + own_sertificate + "']"
            ).text();
            jenis_properti = $(
                "#jenis_properti option[value='" + jenis_properti + "']"
            ).text();
            property_condition = $(
                "#property_condition option[value='" + property_condition + "']"
            ).text();
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success === "1") {
                        credits.data_bangunan.status_sertifikat = htmlEntities(
                            status_sertificate
                        );
                        credits.data_bangunan.sertifikat_atas_nama = htmlEntities(
                            own_sertificate
                        );
                        credits.data_bangunan.provinsi = htmlEntities(
                            provinsi_sertificate
                        );
                        credits.data_bangunan.kota = htmlEntities(
                            kota_sertificate
                        );
                        credits.data_bangunan.kecamatan = htmlEntities(
                            kecamatan_sertificate
                        );
                        credits.data_bangunan.kelurahan = htmlEntities(
                            kelurahan_sertificate
                        );
                        credits.data_bangunan.kode_pos = htmlEntities(
                            kode_pos_sertificate
                        );
                        credits.data_bangunan.alamat = htmlEntities(
                            alamat_lengkap_sertificate
                        );
                        credits.data_bangunan.jenis_properti = htmlEntities(
                            jenis_properti
                        );
                        credits.data_bangunan.tipe_sertifikat = htmlEntities(
                            own_sertificate
                        );
                        credits.data_bangunan.kondisi = htmlEntities(
                            property_condition
                        );
                        credits.data_bangunan.is_dihuni = htmlEntities(
                            property_condition
                        );
                        cb();
                    }
        ...
        $("#button1").on("click", function(e) {
            ...
            if (
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    error: function(data){
                        $(".label-cekLogin").html("Maaf Server Sedang Bermasalah. Silahkan Coba Lain Waktu").css("color","red").removeClass("hide");
                    },
                    fail: function(data){
                    },
                    success: function(data) {
    ...
    function getmobilormotor(element, params) {
        ...
        var type_kendaraan_placeholder = $("#type_kendaraan").attr(
            "placeholder"
        );
        var merk_kendaraan_placeholder = $("#merk_kendaraan").attr(
            "placeholder"
        );
        $("#merk_kendaraan").empty();
  
        var model_kendaraan_placeholder = $("#model_kendaraan").attr(
            "placeholder"
        );
        var tahun_kendaraan_placeholder = $("#tahun_kendaraan").attr(
            "placeholder"
        );
    ...
    $("#chkSameAddress").change(function() {
        ...
        if ($(this).is(":checked")) {
            ...
            $("#kecamatan_sertificate")
                .empty()
                .select2({
                    placeholder: $("#kecamatan_sertificate").attr(
                        "placeholder"
                    ),
            ...
            $("#kelurahan_sertificate")
                .empty()
                .select2({
                    placeholder: $("#kelurahan_sertificate").attr(
                        "placeholder"
                    ),
            ...
            $(
                "#kota_sertificate, #kecamatan_sertificate, #kelurahan_sertificate, #kode_pos_sertificate, #alamat_lengkap_sertificate"
            ).prop("disabled", true);
  ...
  function tidyArticle() {
    ...
    $(".article-content").find("table").wrapAll("<div class='table-responsive'/>");

    $(".article-content").wrapInner('<div class="row">');
    ...
    $(".article-content img").each(function (index) {
        ...
        if ($(this).next("span").length) {
            $(this).next("span").addBack().wrapAll('<div class="col-12 col-md-6 col-xs-12 flexItem" style="padding:10px; text-align:center">');
        } else {
